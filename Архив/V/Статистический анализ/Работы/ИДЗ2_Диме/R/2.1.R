...
hist (data, right = TRUE, freq = TRUE, col='blue')
h1 <- hist(data, breaks = c(-14.73,-10.73,-6.73,-2.73,1.27,5.27,9.27,13.27,17.27,21.27,25.27,29.27,33.27), col='green')
...
T2 <- array(dim=2)
T2[1] <- (n*Disp)/qchisq((1-(alpha2/2)),n-1)
T2[2] <- (n*Disp)/qchisq((alpha2/2),n-1)
T2
...
P<-function(a){
  p <- 0
  p[1] <- pnorm(-6, a[1], a[2])
  p[2] <- pnorm(0, a[1], a[2]) - sum(p)
  p[3] <- pnorm(7, a[1], a[2]) - sum(p)
  p[4] <- pnorm(13, a[1], a[2]) - sum(p)
  p[5] <- 1 - sum(p)
  p}
