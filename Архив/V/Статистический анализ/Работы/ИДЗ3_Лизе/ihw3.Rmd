...
##### Таблица 1. $\alpha_1 = 0.05; h = 2.30$

```{r echo=FALSE}
data1 <- c(7.35, 12.00, 0.55, 8.63, 7.83, 7.17, 6.25, 9.55, 14.33, 8.98, 9.15, 13.19, 6.92, 10.85, 1.50, 3.85, 11.55, 11.31, 5.97, 8.75, 8.12, 11.46, 7.14, 11.10, 13.87, 11.47, 9.07, 5.28, 11.22, 13.78, 11.74, 9.45, 18.86, 18.22, 9.94, 1.80, 3.94, 13.68, 17.72, 9.44, 10.66, 16.60, 3.19, 8.26, 8.94, 6.32, 6.71, 6.35, 4.87, 7.75)
data2 <- c(5, 1, 3, 3, 3, 6, 3, 3, 2, 3, 2, 5, 3, 1, 2, 3, 5, 1, 2, 3, 4, 4, 4, 3, 3, 3, 3, 2, 0, 4, 3, 2, 3, 5, 5, 2, 5, 4, 3, 2, 4, 2, 3, 2, 4, 4, 3, 2, 2, 3)
...
| Интервал | $(-\infty; -4.6)$ | $(-4.6; -2.3)$ | $(-2.3; 0)$ | $(0; 2.3)$ | $(2.3; 4.6)$ | $(4.6; \infty)$ |
| -------- | ----------------- | -------------- | ----------- | ---------- | ------------ | --------------- |
| $m_i$    | 6                 | 11              | 9          | 11         | 7            | 6               |
...
P <- function(a){
  p<-0
  p[1] <- pnorm(-4.6, 0, a)
  p[2] <- pnorm(-2.3, 0, a) - sum(p)
  p[3] <- pnorm(0, 0, a) - sum(p)
  p[4] <- pnorm(2.3, 0, a) - sum(p)
  p[5] <- pnorm(4.6, 0, a) - sum(p)
  p[6] <- 1 - sum(p)
  p}
